...
\end{equation}
% \begin{equation}
% \text{Accptance Ratio} = \frac{	\prod_a \prod_{a \neq r} \sigma\left(b^{(c)'} - \mathbf{\gamma}^{(c)'} \cdot x^{(ar)}  - ||\mathbf{s}_a^{(c)'} - \mathbf{s}_r^{(c)'}|| \right)}{\prod_a \prod_{a \neq r} \sigma\left(b^{(c)} - \mathbf{\gamma}^{(c)} \cdot x^{(ar)}  - ||\mathbf{s}_a^{(c)} - \mathbf{s}_r^{(c)}|| \right)}
% \end{equation}
